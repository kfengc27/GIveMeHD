 ...
 *
 */
public class Enemy extends Entity implements PlayerObserver, Killer, Runnable{
    ...
    */
	...
	public void run() {
		...
		try {
			System.out.println("The enemy is " + this.stop + "be killed");
			while(!this.stop) {
			if(this.dungeon.getRunning() == true) {
//				this.killPlayer();
				
				Thread.sleep(dungeon.getSpeed()*2);
				if(this.stop == true) {
...
//						System.out.println("the player should move Up");
				...
				System.out.println("The game is stopped. Sorry i can not move now.");
			}
		}
	...
	/**
	 * notify if player is invincible
	 */
	@Override
